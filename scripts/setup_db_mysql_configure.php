<?php
	...
	// Start the MySQL server process in the background without privileges.
	...
	if (!$procresult["success"])  CLI::DisplayError("Unable to start MySQL server.", $procresult);

	// Wait for the UNIX socket to come up.
	echo "Waiting for MySQL UNIX socket...";
	fflush(STDOUT);
	$retries = 30;
	do
	{
	...
	if (!$retries)
	{
		...
		$failedmsg = "MySQL failed to start within 30 seconds.";
	}
	else
	{
		sleep(1);
		...
		if (!isset($installconfig["accounts"]["mysql"]))  $installconfig["accounts"]["mysql"] = array("root" => MakePassword());
		...
		try
		{
			...
			echo "Setting MySQL root password, securing the root account, and committing privilege changes...\n";
			$db->Query(false, "FLUSH PRIVILEGES");
			$db->Query(false, "ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY ?", $installconfig["accounts"]["mysql"]["root"]);
		...
		catch (Exception $e)
		{
			...
			$failedmsg = "The MySQL connection or a query failed.";
		}
	}

	// Stop the privilege-less MySQL server.
	echo "Terminating child MySQL process...";
	fflush(STDOUT);
